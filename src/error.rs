...
/// Alias for a `Result` with the error type `tjson::Error`.
...
/// Categorizes the cause of a `tjson::Error`.
...
impl From<Error> for io::Error {
    /// Convert a `tjson::Error` into an `io::Error`.
    ///
    ...
    /// }
    ///
    /// impl From<tjson::Error> for MyError {
    ///     fn from(err: tjson::Error) -> MyError {
    ///         use tjson::error::Category;
    ///         match err.classify() {
    ///             Category::Io => {
    ///                 MyError::Io(err.into())
    ///             }
